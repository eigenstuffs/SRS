...
@onready var ROD_TYPE : FishingRodTypes = load("res://scenes/minigames/fishing/Inventory/types_of_rod.tres")
...
func show_description(rod_type : int):
	...
	reel_strength.text = str(ROD_TYPE.rod_list[rod_type].get("reel_strength"))
	reel_size.text = str(ROD_TYPE.rod_list[rod_type].get("reel_size"))
