...
@export var end_position_z = 3.95

@onready var audio_synchronizer : AudioSynchronizer = $AudioSynchronizer

func _ready() -> void:
	audio_synchronizer.spawn_offset_seconds = (end_position_z - start_position_z) / note_speed
	audio_synchronizer.stream = stream
	audio_synchronizer.start()

func spawn_note(info : BeatmapParser.ObjectInfo, offset : float) -> void:
